...
const myProjects = [
  {
    title: "MedLink.AI",
    description: "Built a first-aid assistant that actually works. React frontend talks to Flask backend, integrated Gemini API for smart medical guidance.",
    tech: ["React", "Flask", "Gemini API"],
    link: ["https://medlink-ai-lrf1.onrender.com"]
  },
  {
    title: "LeetCode Grind",
    description: "365 days of grinding DSA problems in C++. It's brutal, but necessary. Every problem solved makes me a better engineer.",
    tech: ["C++", "DSA", "MySQL"],
              ...
              <div className="card h-100 custom-card p-3 shadow-sm">
                <div className="card-body d-flex flex-column">
                  <div className="d-flex justify-content-between align-items-start mb-3">
                    <h5 className="card-title fw-bold text-white mb-0">{project.title}</h5>
                    {/* External Link Icon for better UX */}
                    <span className="text-warning small">↗</span>
                  </div>
                  
                  <p className="card-text text-secondary-custom small">{project.description}</p>

                  <div className="mt-auto pt-3">
                    {project.tech.map((tag, i) => (
                      <span key={i} className="badge rounded-pill badge-outline me-2">
                        {tag}
                      </span>
                    ))}
                  </div>
